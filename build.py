...
for r in rows:
    ...
    content = f'''<main id="main" class="detail"><a class="back" href="../index.html">← All records</a><p class="eyebrow">RECORD {rid} · {e(r['category'])}</p><h1>{e(r['name'])}</h1><div class="record-meta">{badge(tier)}<span>Original classification</span><span class="review-status">{e(r['review_status'])}</span></div><div class="actions">{project}<a class="button" href="../data/{rid}.json">View record data</a></div><div class="detail-grid"><article><section class="panel"><p class="eyebrow">SOURCE REVIEW</p><h2>What the reviewed material shows</h2><p class="description">{e(r['description'])}</p></section><section class="panel"><h2>Scope and limitations</h2><ul>{limits}</ul></section><section class="panel"><h2>Source snapshots</h2><ul class="sources">{sources}</ul><p class="muted">Repository links identify the material reviewed. A source observation does not certify a shipped release.</p></section></article><aside><section class="panel facts"><h2>At a glance</h2><dl><dt>Original tier</dt><dd>{tier} · {LABELS[tier]}</dd><dt>Reviewed channel</dt><dd>{e(r['channel'])}</dd><dt>Primary-updater assessment</dt><dd>{e(proposed if proposed != 'UNRESOLVED' else 'Not established')}</dd><dt>Record snapshot</dt><dd>{e(r['snapshot_date'])}</dd><dt>Classification certified?</dt><dd>{'Yes' if r['classification_certified'] else 'No — read the scoped review'}</dd></dl></section><section class="panel tier-note tier-{tier}"><h2>What tier {tier} means</h2><p>{MEANINGS[tier]}</p><a href="../about.html">Read the definitions →</a></section><section class="panel"><h2>Something missing or incorrect?</h2><p>Help improve this record with the affected version, update channel, and supporting evidence.</p><div class="actions vertical">{actions}</div></section></aside></div></main>'''
...
about += '<section class="panel"><h2>What these counts mean</h2><p>This collection contains 3,006 historical records, including duplicate projects and separate platforms and installation channels. The original A–D labels are retained for comparison. Later source reviews may narrow a finding or contradict its original tier; read each record’s description and primary-updater assessment.</p><p>The sample was assembled for breadth, not randomly selected. N records have not received the same follow-up as A–D records. Neither the counts nor agreement between AI reviewers establishes a population-wide security rate.</p><h2>How the review worked</h2><p>AI agents inspected source in batches. Follow-up reviewers challenged the claims and recorded corrections, bounded observations, and evidence gaps. The study author reports manually reviewing Tier D. This website presents saved observations; it does not test live update services.</p><h2>Important distinctions</h2><p>A user-initiated update can still install code. A browser link is notification-only. Plugins, firmware, and application updates need separate scopes. HTTPS with proper validation protects transport; a checksum from the same compromised distribution service does not independently authenticate a release. Signature enforcement does not protect against every compromise of the build or authorized release process.</p></section></main>'
